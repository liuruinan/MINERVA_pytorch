...
import numpy as np
from main_code.data.feed_data import Batcher
from main_code.data.grapher import EntityGrapher
import logging
import torch
...
class Episode(object):
    ...
    def __init__(self, graph, data, params):
        ...
        #action[:,:,0] =entity, [:,:,1] = relation, [:,:,2] logical
        next_actions = self.grapher.return_next_actions(self.current_entities, self.start_entities, self.query_relation,
                                                        ...
                                                        self.num_rollouts)
        # next_logical_action = self.grapher.return_logical_action()
        self.state = {}
...
class env(object):
    def __init__(self, params, mode='train'):
        ...
        if mode == 'train':
            self.batcher = Batcher(input_dir=input_dir,
        ...
        self.grapher = EntityGrapher(triple_store=params['data_input_dir'] + '/' + 'graph.txt',
